...
class PathGrammar(Grammar):
    ...
    def __init__(self, transcript_file_or_list, label2index_map):
        ...
        for transcript in transcripts:
            ...
            for i in range(1, len(transcript)):
                ...
                past = self.successors.get(transcript[i-1], list())
                past.append(transcript[i])
                self.successors[transcript[i-1]] = past
    ...
    def score(self, context, label):
        if label in self.possible_successors(context):
            ...
            return -np.inf
        # succ_num = len(context)
        # label_num = self.successors.get(context, list())
